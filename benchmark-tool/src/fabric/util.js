		...
		.then((user) => {
			return new Promise((resolve, reject) => {
				...
				return cop.enroll({
					...
					logger.log('Failed to enroll and persist user. Error: ' + (err.stack ? err.stack : err));
				});
			});
		});
}
...
function getAdmin(client, t, userOrg) {
	...
	var keyPath = path.join(__dirname, rootpath, cryptodir, ORGS[userOrg].adminCryptoPath["keyPath"])
	// util.format('../../%s/peerOrganizations/%s.example.com/users/Admin@%s.example.com/keystore', cryptodir, userOrg, userOrg));
	if (!fs.existsSync(keyPath)) {
		keyPath = path.join(__dirname, rootpath, cryptodir, ORGS[userOrg].adminCryptoPath["keyPath"])
	}
	var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	var certPath = path.join(__dirname, rootpath, cryptodir, ORGS[userOrg].adminCryptoPath["certPath"])
	//path.join(__dirname, util.format('../../%s/peerOrganizations/%s.example.com/users/Admin@%s.example.com/signcerts', cryptodir, userOrg, userOrg));
	if (!fs.existsSync(certPath)) {
		certPath = path.join(__dirname, rootpath, cryptodir, ORGS[userOrg].adminCryptoPath["certPath"])

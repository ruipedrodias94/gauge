...
class MonitorDocker extends MonitorInterface {
    ...
    getPeers() {
        ...
        for(let i in this.containers) {
			//console.log("Container: ",this.containers[i])
...
function findContainers() {
    ...
    var promises = [];
	//console.log("filterName.local: ",filterName.local)
	...
	//console.log("filterName.remote: ",filterName.remote)
	...
	//	var socket = '/var/run/docker.sock';
		...
		//}
	//	var docker = new this.Docker({ socketPath: socket,host: h,port: filterName.remote[h].port});
        ...
        let p = docker.listContainers().then((containers) => {
					...
					//console.log(filterName.remote[h].containers)
					//console.log(filterName.remote[h].containers.indexOf('all'))
					...
					//	console.log('else')
                        ...
                        for(let i = 0 ; i < size ; i++) {
								//console.log("cointainername: ",containers[i].Names[0])
							//	console.log("filterName.remote[h].containers: ",filterName.remote[h].containers)
                            if(filterName.remote[h].containers.indexOf(containers[i].Names[0]) != -1) {
								//console.log("cointainername: ",containers[i].Names[0])
								//console.log("filterName.remote[h].containers: ",filterName.remote[h].containers)
								//console.log("containername: ",filterName.remote[h].containers.indexOf(containers[i].Names[0]))
                                let container = docker.getContainer(containers[i].Id);
                                this.containers.push({id: containers[i].Id, name: h + containers[i].Names[0], remote: container});
                                this.stats[containers[i].Id] = newContainerStat();
                            }
                    ...
                    console.log("Error(monitor-docker)logged:" + err);
                    return Promise.resolve();
                });
        promises.push(p);
    }

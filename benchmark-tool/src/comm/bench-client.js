...
function doTest(msg) {
    ...
    return blockchain.getContext(msg.cmd)
        .then((context) => {
            ...
            return rounds.reduce(function (prev, item, index) {

                return prev.then(() => {
                    promises.push(cb.run(index + 1));
                    idx++;
                ...
                .then((result) => {
                    if (msg.cmd != "query" && !msg.args['query']) {
                        return blockchain.releaseContext(bcContext).then(() => {
                            return blockchain.getResultConfirmation(result, result.length)
                        })
                    } else {
                        return blockchain.releaseContext(bcContext).then(() => {

...
var channel_number = process.argv[2]
var targets=[]
var peers=[]
var promises=[]
...
var channelName = Client.getConfigSetting('fabric').channel[channel_number].name;
...
console.log("connecting to ", ORGS.org1.peer1["server-hostname"])
...
Client.newDefaultKeyValueStore({path:"../../hfc/hfc-test-kvs_peerOrg2"}).then((store) => {
    ...
    return getAdmin(client,"org1", ORGS.org1.mspid);
    
}).then((admin) => {
	console.log("successfully enrolled admin!!")
	client._userContext = admin
    getBlockHeight().then((height)=>{
        console.log("height",  height.height.low) 
        var counter
        getBlockInfo(counter, height.height.low-1)
    })
...
function getBlockInfo(counter,total){
        ...
        if(counter>total)
          ...
          .then((res)=>{
            ...
            console.log("Length", res.metadata.metadata[2].length)
            console.log("The metadata array", res)
...
function getBlockHeight(){
    ...
    return channel.queryInfo(peer_obj, true)
}
